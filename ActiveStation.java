		...
		if(trains.size() >= PLATFORMS){
			...
			while(pIter.hasNext()){
				...
				try {
					...
					logger.info("Passenger "+p.id+" carrying "+p.getCargo() +" kg cargo embarking at "+this.name+" heading to "+p.destination.name);
					pIter.remove();
				} catch (Exception e){
			...
			for(Passenger p: ps){
				try {
					t.embark(p, this.type);
					logger.info("Passenger "+p.id+" carrying "+p.getCargo() +" kg embarking at "+this.name+" heading to "+p.destination.name);
				} catch(Exception e){
					this.waiting.add(p);
				}
			}
